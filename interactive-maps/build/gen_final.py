...
TPL = r"""<!-- 東京防災旅遊互動地圖 — 環境資訊中心
     Leaflet + 內嵌行政區 GeoJSON（都縣界＋東京23區界＋鄰縣市町村界；無圖磚＝無道路，只留行政交界＋水域）。
     繁中地名（自動位移避讓、移太多才刪）、圖釘旁名稱、多種 mapkawaii 風配色切換、鎖死靜態 960x640、可 <iframe> 內嵌。
     界線/pin/地名皆以 Python 算圖核對。座標/文案在 spots、地名在 places、配色在 THEMES。 -->
<meta charset="utf-8">
...
<style>
  ...
  :root[data-theme="mint"]{ --paper:#eef6f0; --water:#a9d4d0; --bodybg:#dcebe4;
    --label:#6f9683; --labelbig:#547a66; --sealbl:#3f8a86; --accent:#4fb0a0 }
  :root[data-theme="sakura"]{ --paper:#fbeef2; --water:#cdd9ea; --bodybg:#f3e0e8;
    --label:#b27f92; --labelbig:#a2687d; --sealbl:#7f8fc0; --accent:#e0679a }
  :root[data-theme="kraft"]{ --paper:#efe2c8; --water:#bcc9c0; --bodybg:#e2d3b4;
    --label:#8a7550; --labelbig:#6f5a38; --sealbl:#5f7d6a; --accent:#c0663f }
  *{box-sizing:border-box}
  ...
  .titlebar p{ margin:1px 0 0; font-size:11px; color:#a2957f }
  .legend{
  ...
  .themes{
    position:absolute; right:14px; bottom:14px; z-index:6; display:flex; gap:7px;
    background:rgba(255,255,255,.9); backdrop-filter:blur(3px);
    padding:7px 9px; border-radius:20px; border:2px solid #fff; box-shadow:0 5px 16px rgba(90,70,40,.16);
  }
  .sw{ width:22px; height:22px; border-radius:50%; cursor:pointer; border:2px solid #fff;
    box-shadow:0 1px 3px rgba(0,0,0,.25); outline:2px solid transparent; transition:outline-color .15s }
  .sw[data-on="1"]{ outline-color:#8a7d66 }
  ...
  .pin-name{
    position:absolute; left:27px; top:0; white-space:nowrap; font-size:12px; font-weight:800;
  ...
  .card img.photo{ width:100%; height:108px; object-fit:cover; border-radius:8px; margin:7px 0; display:block; background:#eee }
  ...
  /* 手機版：縮小 overlay、地名字級，資訊卡加寬，避免小螢幕擁擠 */
  @media (max-width:600px){
    .titlebar, .legend, .themes{ display:none }   /* 手機版：拿掉標題/圖例/色票；說明卡改「點圖釘才出」的左下小卡 */
    .rlabel{ font-size:11px; letter-spacing:0 } .rlabel.big{ font-size:13px; letter-spacing:1px }
    .pin-anchor{ transform:scale(.8) }   /* 手機：圖釘＋名稱再小一點（繞底尖縮放、尖點仍對準座標），免重疊 */
    .pin-name{ font-size:11px; left:25px } .pin-name.left{ right:25px }
    .info{ left:8px; bottom:8px; width:auto; max-width:44%; padding:8px; font-size:11px }   /* 手機：說明卡寬度再縮 1/4 */
    .card img.photo{ height:64px; margin:5px 0 } .card h3{ font-size:12.5px }
  }
  /* 真手機（觸控、非滑鼠）：隱藏詳細說明欄，只留分類/照片/名稱——拉視窗預覽不受影響 */
  @media (max-width:600px) and (pointer:coarse){
    .card p.desc{ display:none }
    .card .area{ display:none }   /* 真手機：popup 不顯示地區（東京・大田 咖啡色字） */
  }
</style>
...
// 多種 mapkawaii 風配色（地名規則不變，只換顏色）
const THEMES = {
  washi:  { name:'瓦紙',   land:'#f4eee0', pref:'#c8a98f', muni:'#c3b39a', water:'#b6d3df' },
  mint:   { name:'薄荷海', land:'#eaf4ec', pref:'#8fb8a0', muni:'#aecabb', water:'#a9d4d0' },
  sakura: { name:'櫻',     land:'#fbeef2', pref:'#e0a8bc', muni:'#e6c6d3', water:'#cdd9ea' },
  kraft:  { name:'牛皮紙', land:'#efe4cc', pref:'#b09462', muni:'#cbb890', water:'#bcc9c0' },
};
...
for (const s of spots){
  ...
  const icon = L.divIcon({
    ...
    html:`<div class="pin-anchor" data-spot="${s.n}">
            ...
            <span class="pin-name${s.pinLeft?' left':''}${s.pinTR?' tr':''}" data-spot="${s.n}">${s.short||s.zh}</span>
          </div>`
  });
  const photo = `https://picsum.photos/seed/${encodeURIComponent(s.zh)}/320/180`;
  const html =
    `<div class="card">
...
function refit(){
  const small = window.matchMedia('(max-width:600px)').matches;   // 跟 CSS 斷點一致、不靠 Leaflet 初始尺寸
  const coarse = window.matchMedia('(pointer:coarse)').matches;   // 真手機（觸控）
  let P;
  if (small){
    const shift = coarse ? map.getSize().x / 6 : 0;   // 真手機：地圖往右移 1/6
...
function carMs(){ return (isSmall() && window.matchMedia('(pointer:coarse)').matches) ? 2750 : 4200; }   // 真手機 2.75s、其餘 4.2s
...
function applyTheme(name){
  ...
  const t = THEMES[name];
  landLayer.setStyle({ fillColor:t.land, color:t.pref });
  muniOther.setStyle({ color:t.muni });
  muniKu.setStyle({ color:t.muni });
  document.querySelectorAll('.sw').forEach(s => s.dataset.on = s.dataset.theme===name ? '1':'0');
}
const themebar = document.getElementById('themes');
for (const [key,t] of Object.entries(THEMES)){
  const sw = document.createElement('div');
  sw.className = 'sw'; sw.dataset.theme = key; sw.title = t.name;
  sw.style.background = t.land; sw.style.borderColor = t.water;
  sw.addEventListener('click', () => applyTheme(key));
  themebar.appendChild(sw);
}
applyTheme('washi');
</script>
"""
